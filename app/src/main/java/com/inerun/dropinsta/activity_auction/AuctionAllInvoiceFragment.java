...
//        DropInsta.serviceManager().postRequest(UrlConstants.URL_READY_INVOICE_LIST, params, null, response_listener, response_errorlistener, "101");
		...
		custInvoiceParcelDataList = SQLite.select().from(AuctionInvoice.class).where(AuctionInvoice_Table.status.eq(1)).queryList();
//        Gson gson = new Gson();
...
//            Drawable mDivider = getDrawable(R.drawable.payment_line_divider);
            ...
            adapter.setOnItemClickListener(new AuctionInvoiceListAdapter.OnItemClickListener() {
                ...
                public void onItemClick(View v, int position) {
                    pushFragment(AuctionInvoiceDetailFragment.newInstance(custInvoiceParcelDataList.get(position).getInvoice_number()));
                }
            });
